...
void GameClient::update(sf::RenderWindow &window)
{
	...
	bool worldChanged = m_world.update(wud);

	static sf::Clock heartbeatClock;
    bool const sendHeartbeat = heartbeatClock.getElapsedTime().asMilliseconds() > 100;

    if(worldChanged || sendHeartbeat)
    {
        UnreliablePktType typeToSend = worldChanged ? UnreliablePktType::INPUT : UnreliablePktType::HEARTBEAT;
        sf::Packet pkt = createTickedPkt(typeToSend, m_world.m_clientTick);
        pkt << wud;
