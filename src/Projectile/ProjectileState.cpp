...
ProjectileState::ProjectileState(uint32_t id, sf::Vector2f position, sf::Vector2f velocity, uint32_t ownerId, int damage)
	: m_id(id)
	, m_position(position)
	, m_velocity(velocity)
	, m_ownerId(ownerId)
...
{
	return sf::FloatRect(
		sf::Vector2f(m_position.x - PROJECTILE_SIZE / 2.f, m_position.y - PROJECTILE_SIZE / 2.f),
