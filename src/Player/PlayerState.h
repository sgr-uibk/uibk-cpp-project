...
struct InterpPlayerState
{
    ...
    InterpPlayerState(sf::Vector2f const pos, sf::Angle const rot, sf::Angle const cannonRot) : m_pos(pos), m_rot(rot), m_cannonRot(cannonRot)
    {
    }
...
struct PlayerState
{
    ...
    // Now valid because of forward declaration 'struct MapState;' above
